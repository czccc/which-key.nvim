...
function M.process_motions(ret, mode, prefix_i, buf)
  local operator = mode == "v" and "" or M.get_operator(prefix_i)
  if (mode == "n" or mode == "v") and operator then
    local op_prefix = prefix_i:sub(#operator + 1)
    ...
    if not ret.mapping and op_results.mapping then
      ...
      ret.mapping.prefix = prefix_i -- This is wrong but isn't used anywhere
    end
